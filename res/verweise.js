...
jQuery(document).ready(function($){
	...
	// Reset button
	$('#reset-verweise,#unlink-verweise').click(function(){
	...
	var verweise = {
		...
		check: function( type ) {
			...
			if( type == 'path' ) {
				...
				post['username'] = $('#y_verweise_login').val();
				post['password'] = $('#y_verweise_passwd').val();
			}
			post['action'] = 'verweise-check';
			post['_ajax_nonce'] = $('#_ajax_verweise').val();
			post['verweise_type'] = type;

			$('#check_'+type).html('Pr&uuml;ft die Eingabe...');
			
			$.ajax({
				url : ajaxurl,
				data : post,
				success : function(x) { verweise.check_ok(x, '#check_'+type); },
				error : function(r) { verweise.check_notok(r, '#check_'+type); }
			});
		},
		...
		check_notok : function(r, div) {
			...
			if ( er )
				$(div).html('Fehler durch AJaX-&uuml;ckgabe: '+er);
		}
	};
	
	$(document).ready(function(){
		// Check path & URLs
		$('.verweise_check').click(function(){
...
function update_key_msg() {
	jQuery('#verweise_now_connect').text( 'Klicke jetzt auf "Speichern". Wenn Deine Eingaben korrekt sind, wird hier ein Anmelde-Knopf von Twitter erscheinen.' );
}